...
func getWireguard(oStr string) string {
	data, err := base64.StdEncoding.DecodeString(oStr)
	if err != nil {
		return oStr
	}
...
func GetXrayOutboundFromURI(rawURI string) string {
	...
	if scheme == parser.SchemeWireguard {
		baseVMESSUri := getOutboundJSONIntended(strings.Replace(rawURI, parser.SchemeWireguard, "", 2))
		rawURI = parser.SchemeWireguard + baseVMESSUri
	}
